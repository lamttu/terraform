...
# There are 2 projects defined in the variables.tf so there will be 2 vpcs
...
module "elb_http" {
  ...
  name     = trimsuffix(substr(replace(join("-", ["lb", random_string.lb_id.result, var.project, each.value.environment]), "/[^a-zA-Z0-9-]/", ""), 0, 32), "-")
  internal = false
...
module "ec2_instances" {
  ...
  # The [*] below is a splat expression https://www.terraform.io/language/expressions/splat
